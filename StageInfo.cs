...
namespace StageInfo {
    ...
    public class StageInfo : Mod {
        ...
        public override void OnInitialized() {
            ...
            GameManager.Instance.Game.Messages.Subscribe<OpenEngineersReportWindowMessage>(OpenEngineersReport);
            GameManager.Instance.Game.Messages.Subscribe<CloseEngineersReportWindowMessage>(CloseEngineersReport);
        }
        ...
        private void OpenEngineersReport(MessageCenterMessage msg) => showGUI = true;
        private void CloseEngineersReport(MessageCenterMessage msg) => showGUI = false;
        ...
        private void FillGUI(int windowID) {
            if (refreshing || situData.Count == 0) GUILayout.Label("<i>Calculating Stage Info...</i>");
                ...
                for (int i = 0; i < situData.Count; i++) {
                    ...
                    else GUILayout.Label($" <color=#0A0B0E><b>|</b></color> <color=#C0C7D5>{situData[i].burn:N0} s</color>", GUILayout.Width(windowWidth / 4));
                    GUILayout.EndHorizontal(); }
                GUILayout.BeginHorizontal();
